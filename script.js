const speed = {
  ...
  async function updateTables() {
    ...
    const courseIndex = selectedCourse === "창덕궁" ? 0 : selectedCourse === "경복궁" ? 1 : 2;
    ...
    function dfs(currentPoint, path, distance, time, visited) {
      ...
      for (const linkIdx of currentPoint.link) {
        if (!visited.has(linkIdx)) {
          ...
          const dist = calculateDistance(currentPoint.location, nextPoint.location);
          const travelTime = parseFloat(calculateTime(dist, speed[transportMode]));
  ...
  function createTable(results, transportMode) {
    ...
    headerRow.innerHTML = `<th>경로</th><th>거리 (m)</th><th>${transportMode === 'bicycle' ? '자전거' : '도보'}로 이동시간</th>`;
    table.appendChild(headerRow);
  
    results.forEach(result => {
      const row = document.createElement('tr');
      row.innerHTML = `<td>${result.path.join(' -> ')}</td><td>${result.distance.toFixed(2)}</td><td>${formatTime(result.time)}</td>`;
